...
final class OrdiniMaterialeController extends BaseController
{
    ...
    public function rimuoviMateriale($id_ordine, $id_record)
    {
        ...
        if ($this->ordineMaterialeVoceModel->delete($id_record)) {
            // Aggiorna l'importo totale dell'ordine
            $this->aggiornaImportoTotale($id_ordine);
            
            $session->setFlashdata('success', 'Materiale rimosso dall\'ordine');
        } else {
    ...
    public function importaVociOfferta($id_ordine)
    {
        ...
        // Validazione
        $rules = [
            'id_offerta' => 'required|numeric'
        ];
        
        if (!$this->validate($rules)) {
            return redirect()->to('/ordini-materiale/' . $id_ordine)->with('error', 'Offerta fornitore non specificata');
        }
        
        $id_offerta = (int)$this->request->getPost('id_offerta');
        
        // Verifica che l'offerta esista
        $offerta = $this->offertaFornitoreModel->find($id_offerta);
        if (!$offerta) {
            return redirect()->to('/ordini-materiale/' . $id_ordine)->with('error', 'Offerta fornitore non trovata');
        }
        
        // Ottieni le voci selezionate (se specificate)
        $vociSelezionate = $this->request->getPost('voci_selezionate');
        
        // Importa le voci dall'offerta
        $risultato = $this->ordineMaterialeVoceModel->importaVociOfferta($id_ordine, $id_offerta, $vociSelezionate);
        
        if (count($risultato['importate']) > 0) {
            // Aggiorna l'importo totale dell'ordine
            $this->aggiornaImportoTotale($id_ordine);
            
            $session->setFlashdata('success', 'Importate ' . count($risultato['importate']) . ' voci dall\'offerta');
        } else {
            $session->setFlashdata('info', 'Nessuna voce importata dall\'offerta');
        }
        
        if (count($risultato['errori']) > 0) {
            $session->setFlashdata('error', 'Errori durante l\'importazione: ' . implode(', ', $risultato['errori']));
        }
        
        // Aggiorna il riferimento all'offerta nell'ordine
        $this->ordineMaterialeModel->update($id_ordine, ['id_offerta_fornitore' => $id_offerta]);
    ...
    private function aggiornaImportoTotale($id_ordine)
    {
        ...
        // Ottieni tutte le voci dell'ordine
        $voci = $this->ordineMaterialeVoceModel->where('id_ordine', $id_ordine)->findAll();
        log_message('debug', "Trovate " . count($voci) . " voci per l'ordine");
        ...
        log_message('debug', "Importo totale voci: {$importoVoci}");
        ...
        // Verifica la struttura dell'ordine e i campi disponibili
        log_message('debug', "Struttura completa dell'ordine: " . json_encode($ordine));
